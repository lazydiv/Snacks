    ...
    return (
        ...
        before:backdrop-blur-md'>
            <div className='mx-5 items-center h-full w-full text-2xl flex '>
                ...
                <div className='w-full absoulute md:w-3/6 mx-auto '>
                    <input type="text" onBlur={() => setTimeout(() => {
                        setIsOpen(false)
                    }, 200)} onKeyPress={(e) => e.key === 'Enter' && setSearchValue(event.target.value) & setIsOpen(true)} className={`w-full ${ !visible ? 'invisible' : 'visible mt-32 w-full' }  outline-none md:visible md:mt-0 p-2 items-center bg-mainLightGray opacity-20 focus:opacity-100 rounded-md  text-mainBlack`} placeholder='type here ...'/>
                </div>
